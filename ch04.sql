...
select sum(주문수량) as '주문수량합', sum(단가*주문수량) as '주문금액합' from 주문세부;
...
select 고객번호 , count(*) from 주문 
group by 고객번호
order by count(*) desc
limit 3;

-- 4. 사원 테이블에서 직위별로 사원수와 사원이름목록을 보이시오.
select 직위, count(직위) as '사원수',이름 from 사원
group by 직위,이름;

-- group_concat() : 여러행의 문자열을 결합해줌.
select 직위, count(직위) as '사원수',GROUP_CONCAT(이름 separator ',') from 사원
group by 직위;
